...
const state = reactive({
  recordList: [],
  columns: [
    { prop: 'gameTypeName', label: '游戏名称' },
    { prop: 'totalBetAmt', label: '总投注金额' },
    { prop: 'totalBetValidAmt', label: '总投注有效流水' },
    { prop: 'totalProfit', label: '总输赢金额' },
  ],
  total: 30,
  pageSize: 15,
  current: 1,
...
const agentGameTypeStatisData = () => {
  agentGameTypeStatis(state.formInline).then(item => {
    if (item.code === 200) {
